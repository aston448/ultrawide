...
export const UserDesignUpdateMashScenarios = new Mongo.Collection('userDesignUpdateMashScenarios');

let Schema = new SimpleSchema({
    // Design Identity
    userId:                         {type: String},                         // Meteor user id - must be a user
    designVersionId:                {type: String},                         // Must be a design version
    designUpdateId:                 {type: String},                         // Must be a design update
...
UserDesignUpdateMashScenarios.attachSchema(Schema);


// Publish
if(Meteor.isServer){

    Meteor.publish('userDesignUpdateMashScenarios', function userDesignUpdateMashScenariosPublication(userId){
        return UserDesignUpdateMashScenarios.find({userId: userId});
    })
}
