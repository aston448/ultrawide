...
class WorkPackageServicesClass {
    ...
    closeWorkPackage(workPackageId){

        if(Meteor.isServer) {

            WorkPackageData.setWorkPackageStatus(workPackageId, WorkPackageStatus.WP_CLOSED);
        }
    }

    reopenWorkPackage(workPackageId){
        ...
        if(Meteor.isServer) {
            ...
            // Remove any scenarios that are no longer associated with this WP (might now be in a new WP)
            WorkPackageModules.removeMigratedWpScenarios(workPackageId);
        }
    }
