
export const DefaultItemNames = {

    NEW_DESIGN_NAME:                'New Design',
    NEW_DESIGN_VERSION_NAME:        'First Draft',
    NEW_DESIGN_VERSION_NUMBER:      '0.1',
    NEXT_DESIGN_VERSION_NAME:       'New Updatable Version',
    NEXT_DESIGN_VERSION_NUMBER:     'x.x',
    NEW_DESIGN_UPDATE_NAME:         'New Design Update',
    NEW_DESIGN_UPDATE_REF:          'CR001',
    NEW_WORK_PACKAGE_NAME:          'New Work Package'

};

export const DefaultComponentNames = {

    NEW_APPLICATION_NAME:           'New Application',
    NEW_DESIGN_SECTION_NAME:        'New Design Section',
    NEW_FEATURE_NAME:               'New Feature or Use Case',
    NEW_NARRATIVE_TEXT:             'As a \nI want to \nSo that I can ',
    NEW_FEATURE_ASPECT_NAME:        'New Feature Aspect',
    NEW_SCENARIO_NAME:              'New Scenario',
    NEW_DICTIONARY_ENTRY_NAME:      'New Domain Term'

};

export const DefaultDetailsText = {

    NEW_APPLICATION_DETAILS:        'A high level description of the application may be entered here...',
    NEW_DESIGN_SECTION_DETAILS:     'Text relevant to this section may be entered here...',
    NEW_FEATURE_DETAILS:            'Text relevant to this feature may be entered here...',
    NEW_FEATURE_ASPECT_DETAILS:     '', // No details for feature aspects
    NEW_SCENARIO_DETAILS:           'Text relevant to this scenario may be entered here...',
    NEW_DICTIONARY_ENTRY_TEXT:      'Enter definition for Domain Term here...'
};

export const DefaultLocationText = {
    NEW_TEST_OUTPUT_LOCATION_NAME:              'New Test Output Location',
    NEW_TEST_OUTPUT_LOCATION_FILE_ALIAS:        'Test Output File Alias',
    NEW_TEST_OUTPUT_LOCATION_FILE_NAME:         'Test Output File Name',
};

export const DefaultUserDetails = {
    NEW_USER_NAME:                  'newUser',
    NEW_USER_PASSWORD:              'password',
    NEW_USER_DISPLAY_NAME:          'New User'
};
